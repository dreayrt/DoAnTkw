...
// PopUp sản phẩm từ trang Feature đổ
document.addEventListener('DOMContentLoaded', () => {
  // ... (renderCartFromLocalStorage, các setup khác) ...

  // 1) Đọc param quickView
  const params = new URLSearchParams(window.location.search);
  const titleToShow = params.get('quickView');
  if (!titleToShow) return;

  // 2) Tìm nút .quick-view có đúng data-title
  const selector = `.quick-view[data-title="${titleToShow}"]`;
  const btn = document.querySelector(selector);
  if (btn) {
    // 3) Kích hoạt click để bật modal
    btn.click();
  }
});